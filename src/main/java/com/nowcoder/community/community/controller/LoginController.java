...
public class LoginController implements CommunityConstant {
    ...
    public String activation(Model model, @PathVariable("userId") int userId, @PathVariable("code") String code ){
        int result=userService.activation(userId,code);   //查看激活的状态
    ...
    public void getKaptcha(HttpServletResponse response/*, HttpSession session*/){
        ...
        //验证码的归属
        String kaptchaOwner = CommunityUtil.generateUUID(); //随机生成一个字符串，到时候让他失效就行
        Cookie cookie = new Cookie("kaptchaOwner",kaptchaOwner); //设置Cookie
        cookie.setMaxAge(60);  //设置有效的时间为60秒
        cookie.setPath(contextPath);  //设置为整个项目下都是有效的
        response.addCookie(cookie);   //发送给客户端
        String redisKey = RedisKeyUtil.getKaptchaKey(kaptchaOwner);
        redisTemplate.opsForValue().set(redisKey,text,60, TimeUnit.SECONDS);  //设置存入在redis里面的时间有效期是60秒
    ...
    public String login(String username,String password,String code,boolean rememberme,
       ...
       // String kaptcha = (String) session.getAttribute("kaptcha");
        ...
        if(StringUtils.isNotBlank(kaptchaOwner)){
            String redisKey = RedisKeyUtil.getKaptchaKey(kaptchaOwner);
            kaptcha = (String) redisTemplate.opsForValue().get(redisKey); //取到存储在redis里面的值
        }
