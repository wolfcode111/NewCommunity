...
//
    ...
    public Map<String,Object> login(String username,String password,int expiredSeconds){
     ...
     //   loginTicketMapper.insertLoginTicket(loginTicket);
        ...
        redisTemplate.opsForValue().set(redisKey,loginTicket);  //redis会将这个对象序列化为Json字符串保存
    ...
    public int updateHeader(int userId,String headerUrl){
        ...
        int rows = userMapper.updateHeader(userId,headerUrl); //先更新，因为也有可能会更新失败
        clearCache(userId);  //清除redis里面的缓存
        return rows;
    }
    ...
    private User initCache(int userId){
        User user = userMapper.selectById(userId); //缓存里面没有的时候，就从数据库中去调
        String redisKey = RedisKeyUtil.getUserKey(userId);
    ...
    private void clearCache(int userId){
        String redisKey = RedisKeyUtil.getUserKey(userId);
        redisTemplate.delete(redisKey); //直接删除就行，省得到时候得考虑一些数据新旧的问题
    }
